...
const uri =
  "mongodb+srv://omarelbanna:AtieyhIE9Xj2wCKk@security.yxz4gwr.mongodb.net/?retryWrites=true&w=majority&appName=security";
...
io.on("connection", (socket) => {
  ...
  // socket.emit("readMessage_test", "hamada");
  socket.on("login", async ({ username, hashedPassword }) => {
    const { success, message, user } = await authenticateUser(
      username,
      hashedPassword
    );
    if (success) {
  ...
  socket.on("message", async ({ cyphertext, cypherkey, user }) => {
    ...
    console.log(sender);
    const channel = "readMessage_" + String(user);
